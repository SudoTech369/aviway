...
Class Master extends DBConnection {
	...
	function add_to_cart(){
        ...
        // Simple debug - enable only during debugging
        // error_reporting(E_ALL); ini_set('display_errors',1);
    
        // Ensure user is logged in
        $client_id = $this->settings->userdata('id');
        if(empty($client_id) || !is_numeric($client_id)){
            echo json_encode(['status'=>'failed','msg'=>'Not logged in']);
            return;
        }
        ...
        if($inventory_id <= 0){
            echo json_encode(['status'=>'failed','msg'=>'Invalid inventory selected']);
            return;
        }
        if($quantity <= 0){
        ...
        // Optional: verify inventory exists (and optionally check stock)
        $stmt = $this->conn->prepare("SELECT id, product_id, quantity FROM inventory WHERE id = ?");
        ...
        // if($quantity > $inv_row['quantity']) { echo json_encode(['status'=>'failed','msg'=>'Requested quantity exceeds stock']); return; }
    
        // Use prepared statements to insert/update cart
        // Check existing cart row
        $check = $this->conn->prepare("SELECT id, quantity FROM cart WHERE inventory_id = ? AND client_id = ?");
        $check->bind_param("ii", $inventory_id, $client_id);
        $ok = $check->execute();
        if(!$ok){
        ...
        if($res && $res->num_rows > 0){
            ...
            $upd = $this->conn->prepare("UPDATE cart SET quantity = ?, price = ? WHERE id = ?");
            $upd->bind_param("idi", $new_qty, $price, $row['id']);
            $exec = $upd->execute();
            if(!$exec){
                echo json_encode(['status'=>'failed','msg'=>$this->conn->error]);
                return;
            ...
            // insert new
            $ins = $this->conn->prepare("INSERT INTO cart (client_id, inventory_id, quantity, price) VALUES (?, ?, ?, ?)");
            $ins->bind_param("iiid", $client_id, $inventory_id, $quantity, $price);
            $exec = $ins->execute();
            if(!$exec){
                echo json_encode(['status'=>'failed','msg'=>$this->conn->error]);
                return;
        ...
        $c = $this->conn->prepare("SELECT COALESCE(SUM(quantity),0) as items FROM cart WHERE client_id = ?");
        $c->bind_param("i", $client_id);
        $c->execute();
        $cres = $c->get_result()->fetch_assoc();
        $count = isset($cres['items']) ? (int)$cres['items'] : 0;
